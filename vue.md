...
| ------------------ | --------------------------- | ---------------------------------------------- |
| **`this`的存在性** | 始终指向组件实例            | `setup()`中`this`为`undefined`                 |
| **数据访问**       | `this.dataProperty`         | 通过`ref`/`reactive`创建，`.value`访问基础类型 |
| **事件派发**       | `this.$emit`                | `context.emit`                                 |
| **生命周期钩子**   | 直接使用（如`mounted(){}`） | 引入钩子函数（如`onMounted`）                  |
| **箭头函数兼容性** | 不推荐（`this`指向错误）    | 推荐（无`this`依赖）                           |
...
等待 Vue 完成 DOM 更新再执行回调函数

### 为什么需要 nextTick()？

- 异步更新机制
  Vue 为了优化性能，会将多次数据变化合并在一次更新中执行，这导致数据改变和 DOM 更新之间存在延迟。如果你在数据变化后立即访问 DOM，可能会得到旧的状态。

- 确保 DOM 已经更新
  使用 nextTick()，你可以将代码延迟到 DOM 更新之后再执行，从而确保操作或测量的 DOM 是最新状态。

### nextTick 的底层原理

在 Vue 3 中，nextTick 的核心就是利用 JavaScript 的 微任务（microtask）机制，因为微任务会在当前调用栈执行完毕后、下次渲染之前执行，完美契合“等所有响应式更新、DOM patch 完成”再运行用户回调的需求。

1. Vue 的响应式系统会将数据变更触发的 DOM 更新操作推入一个 ​​ 异步队列 ​​，而不是立即执行。这种机制通过合并多次数据变更，避免频繁触发 DOM 渲染，从而提升性能
2. 当数据变更时，Vue 不会立即更新 DOM，而是将更新操作封装为回调函数加入队列，等待同一事件循环内的所有数据变更完成后批量执行
3. 当等所有响应式更新完成、DOM patch 完成；从而取微任务队列

## 为什么 Proxy 要搭配 reflect 使用

1. 修正 this 指向：避免上下文错乱。
2. 提供无副作用的操作方式 ​​ 来保证响应式系统的稳定性，因为 Reflect API 始终返回布尔值或操作结果 ​​，而原生操作可能抛出异常。
3. Reflect 的 13 个静态方法与 Proxy 的 13 个陷阱一一对应，这种设计哲学使得两者能完美协作，这种对称性确保了代理行为与原生行为的一致性。

## VUE 文件是如何变成浏览器页面

1. ​​ `.vue` 文件被拆解为 `<template>`、`<script>` 和 `<style>` 三个部分。
2. `<template>` 中的 HTML 语法会被转换为 ​​ 虚拟 DOM 的渲染函数 ​​。
3. `<script>` 中的逻辑会被转换为标准的 ES 模块代码，并与模板的渲染函数结合。
4. `<style>` 中的 CSS 会被提取并处理为独立文件,若使用 `scoped` 属性，编译器会为每个选择器添加唯一属性如`.message[data-v-xxx]`
